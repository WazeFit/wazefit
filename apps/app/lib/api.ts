...
const BASE = typeof window !== 'undefined'
  ? (process.env.NEXT_PUBLIC_API_URL || 'https://api.wazefit.com')
  : 'https://api.wazefit.com'
...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  ...
  // No Next.js, o token pode vir do cookie ou do localStorage (client-side)
  if (typeof window !== 'undefined') {
    const token = localStorage.getItem('wf_token')
    if (token) {
      headers['Authorization'] = `Bearer ${token}`
    } else {
      // Fallback: ler do cookie se disponivel
      const cookieToken = document.cookie
        .split('; ')
        .find(row => row.startsWith('wf_token='))
        ?.split('=')[1]
      if (cookieToken) {
