...
import type { PlanoNutricional, Alimento } from '../../lib/api'
...
export function NutricaoAlunoPage() {
  ...
  async function loadPlano() {
    try {
      setLoading(true)
      const planos = await api.nutricao.planos.list()
      if (planos.length > 0) {
        const detail = await api.nutricao.planos.get(planos[0]!.id)
        setPlano(detail)
      }
    } catch (err) {
          ...
          <Card key={ref.id}>
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-white">{ref.nome}</h3>
              ...
              {(ref.alimentos || []).length === 0 ? (
                ...
                <div className="space-y-2">
                  {ref.alimentos.map((al) => (
                    <div key={al.id} className="flex items-center justify-between text-sm border-b border-gray-800 pb-2 last:border-0">
                      <div>
              ...
              <div className="mt-3 pt-2 border-t border-gray-800 flex justify-end text-xs text-gray-500 space-x-3">
      ...
      {plano.observacoes && (
        <Card>
          <div className="p-4">
            ...
            <p className="text-sm text-gray-400 whitespace-pre-wrap">{plano.observacoes}</p>
          </div>
        </Card>
      )}
    </div>
